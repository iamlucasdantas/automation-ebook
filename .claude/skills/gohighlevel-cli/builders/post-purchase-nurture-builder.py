...
da subconta definida em GHL_LOCATION_ID.
...
def _require_location_id() -> str:
    """Location ID vem SEMPRE do ambiente — nunca de um default hardcoded."""
    loc = os.environ.get("GHL_LOCATION_ID", "").strip()
    if not loc:
        print(
            "Erro: GHL_LOCATION_ID não definida.\n"
            "  Defina no .env da raiz do projeto antes de rodar este builder.",
            file=sys.stderr,
        )
        sys.exit(1)
    return loc
...
def main():
    ...
    location_id = _require_location_id()
    ...
    print("  5. Test send each email to seu e-mail de teste.")
