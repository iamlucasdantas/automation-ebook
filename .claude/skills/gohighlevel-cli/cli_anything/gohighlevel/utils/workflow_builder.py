...
def internal_notification_step(name: str, subject: str, body: str,
                               channel: str = "email", **kw: Any) -> dict:
    """Notificação para a equipe (não para o lead)."""
    return {
        "id": _uid(), "type": "internal_notification", "name": name,
        "attributes": {
            "channel": channel, "subject": subject, "body": body,
            "toType": "assignedUser", "attachments": [],
        }, **kw,
    }


def update_field_step(name: str, field_key: str, value: str, **kw: Any) -> dict:
    """Grava valor em custom field do contato (ex.: CF_DRIVE_LINK)."""
    return {
        "id": _uid(), "type": "update_contact_field", "name": name,
        "attributes": {"fields": [{"key": field_key, "value": value}]}, **kw,
    }


def note_step(name: str, body: str, **kw: Any) -> dict:
    return {
        "id": _uid(), "type": "add_notes", "name": name,
        "attributes": {"note": body}, **kw,
    }


def add_to_workflow_step(name: str, workflow_id: str, **kw: Any) -> dict:
    """Workflow chaining — encadeia um workflow no outro."""
    return {
        "id": _uid(), "type": "add_to_workflow", "name": name,
        "attributes": {"workflowId": workflow_id, "eventStartDate": ""}, **kw,
    }
...
class CampaignBuilder:
    ...
    def build(
        ...
        With `strict=True` (default) a failed pre-flight validation aborts the
        build — creating half-broken workflows in production is worse than
        creating none.
        """
    ...
    def format_summary(self) -> str:
        ...
        if self.stats.get("aborted"):
            lines = ["Build ABORTADO na validação — nada foi criado no GHL:"]
        ...
        if wf_ids:
            domain = ghl_client.app_domain()
            lines.append("\nLinks (rascunho — publique manualmente após revisar):")
            for key, wf_id in sorted(wf_ids.items()):
                lines.append(
                    f"  {key}: https://{domain}/v2/location/{self.loc}/automation/workflows/{wf_id}"
                )
